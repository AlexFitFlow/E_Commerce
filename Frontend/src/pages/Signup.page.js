...
try {
    const user = await emailPasswordSignup(form.firstName, form.lastName, form.email, form.password);
    if (user) {
    redirectNow();
    }
} catch (error) {
...
return <>
        ...
        {/* Sign Up Card */}
        <Card className="card shadow-lg box-shadow3" sx={{ maxWidth: 600 }} style={{display: 'block', margin: 'auto', marginBottom: '5rem', marginTop: '5rem' }}>
